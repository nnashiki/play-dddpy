...
# プロジェクトを作成
project_data = {'name': 'Test Project', 'description': 'テストプロジェクト'}
...
try:
    ...
    if response.status_code == 201:
        ...
        print(
            f'Projects: {json.dumps(list_response.json(), indent=2, ensure_ascii=False)}'
        )

        # Todoを追加
        todo_data = {'title': 'テストタスク', 'description': 'これはテストタスクです'}
        todo_response = requests.post(
            f'http://localhost:8000/projects/{project_id}/todos', json=todo_data
        )
        print(f'\nTodo追加 Status Code: {todo_response.status_code}')
        print(
            f'Todo Response: {json.dumps(todo_response.json(), indent=2, ensure_ascii=False)}'
        )
