...
def test_update_todo_through_project_success():
    ...
    result = usecase.execute(str(todo.id.value), update_dto)
...
def test_update_todo_through_project_todo_not_found():
    ...
    with pytest.raises(ProjectNotFoundError):
        usecase.execute(todo_id, update_dto)
    
    mock_repository.find_project_by_todo_id.assert_called_once()
    mock_repository.save.assert_not_called()


def test_update_todo_uses_find_project_by_todo_id():
    ...
    # Execute
    usecase = UpdateTodoThroughProjectUseCaseImpl(mock_repository)
    usecase.execute(str(todo.id.value), update_dto)
...
def test_update_todo_with_dependencies():
    ...
    # Execute
    usecase = UpdateTodoThroughProjectUseCaseImpl(mock_repository)
    result = usecase.execute(str(todo3.id.value), update_dto)
