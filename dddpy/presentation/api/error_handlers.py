...
from dddpy.domain.todo.exceptions.todo_already_started_error import TodoAlreadyStartedError
...
def _create_error_response(status_code: int, message: str, error_type: str | None = None) -> JSONResponse:
...
def add_exception_handlers(app):
    ...
    async def handle_todo_removal_blocked(request: Request, exc: TodoRemovalNotAllowedError):
        return JSONResponse(status_code=HTTP_400_BAD_REQUEST, content={'detail': str(exc)})
    ...
    async def handle_dep_not_complete(request: Request, exc: TodoDependencyNotCompletedError):
        return JSONResponse(status_code=HTTP_400_BAD_REQUEST, content={'detail': str(exc)})
    ...
    async def handle_already_completed(request: Request, exc: TodoAlreadyCompletedError):
        return JSONResponse(status_code=HTTP_400_BAD_REQUEST, content={'detail': str(exc)})
    ...
    async def handle_duplicate_todo_title(request: Request, exc: DuplicateTodoTitleError):
        return JSONResponse(status_code=HTTP_400_BAD_REQUEST, content={'detail': str(exc)})
    ...
    async def handle_too_many_dependencies(request: Request, exc: TooManyDependenciesError):
        return JSONResponse(status_code=HTTP_400_BAD_REQUEST, content={'detail': str(exc)})
